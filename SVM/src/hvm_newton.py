...
def conjugateGradient(X, I, grad, lambda_, tol=1e-1, max_iter=100):
    """this function is the modified version of conjugateGradient provided by TA, using scipy form matrix calculation
    ...
    Returns:
        :d where d is the solution of: Hd = -g
    ...
    for cg_iter in range(max_iter):
        ...
        d = d + p.multiply(alpha)
        r = r - Ap.multiply(alpha)
        rsnew = r.T.dot(r)
        if np.sqrt(rsnew.toarray()) < tol:
            break
        p = r + p.multiply((rsnew / rsold))
        rsold = rsnew

    return d, cg_iter + 1

def eval(eph,w,grad_norm,X_test,y_test,X_train, y_train,f_opt,time_used,output_file):
    ...
    f_test = f_test_first + f_test_second
    ...
    f_val = f_val_first + f_val_second
    
    f_rel_diff = (f_val-f_opt)/f_val
    
    print(f"eph: {eph}\ntest_accuracy: {test_accuracy}\ngrad_norm: {grad_norm}\nval_f: {f_val}\ntest_f*: {f_test}\nf_rel_diff: {f_rel_diff}\ntime_used: {time_used}s")
    print(f"eph: {eph} test_accuracy: {test_accuracy} grad_norm: {grad_norm} val_f: {f_val} test_f*: {f_test} f_rel_diff: {f_rel_diff} time_used: {time_used}",file = output_file)
#---------------------------------------------------------------------------------------------------------------------------#    
...
if(n==57847):
    ...
    epoch_max = 30
    f_opt = 669.664812
    file_name="result/newton_output_real.txt"
else:
    lamda = 3631.3203125
    epoch_max = 10
    f_opt = 2541.664519
    file_name="result/newton_output_cov.txt"
...
#start training
print("\nstart newton training\n")
print(f"the result will be stored in: {file_name}")
start_t = time.perf_counter()   
for eph in range(1,epoch_max+1):
    
    multiply_result = X_train.dot(w)            
    judge = y_train.multiply(multiply_result)
    judge = judge.toarray()
    index_I = np.argwhere(judge.squeeze()<1).squeeze()
    a = X_train[index_I].dot(w)- y_train[index_I]
    b = X_train[index_I].transpose().dot(a)
    grad = (2*lamda/n)*b + w
    d,iter_r = conjugateGradient(X_train, index_I, grad, lamda,max_iter=100)
    w = w + d
    grad_norm=np.linalg.norm(grad.toarray(), ord=2, axis=None, keepdims=False)
    temp_t = time.perf_counter()
    time_used = temp_t-start_t
    eval(eph,w,grad_norm,X_test,y_test,X_train, y_train,f_opt,time_used,output_file)
