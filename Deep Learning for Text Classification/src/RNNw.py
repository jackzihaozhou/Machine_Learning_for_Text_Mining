'''
Here is training process for RNN w pretrained embed
'''
...
def RNNw():
    mkdir("result")
    mkdir("model")
    ftest_loss = open("result/LSTMw_test_loss.txt","w")
    ftest_acc = open("result/LSTMw_test_accuracy.txt","w")
    ftrain_loss = open("result/LSTMw_train_loss.txt","w")
    ftrain_acc = open("result/LSTMw_train_accuracy.txt","w")
    ...
    config =  LSTMconfiguration(vocab_size= 10000,embedding_size = 100, hidden_size = 100, num_layers = 1, num_class = 1,sentence_length = 300, maxpool_kernel = 4,batch_size = 10,pre_embed = True)   
