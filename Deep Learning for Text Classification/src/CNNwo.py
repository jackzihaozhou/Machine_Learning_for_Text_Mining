'''
Here is training process for CNN w/o pretrained embed
'''
...
import time
from module import TextCNN,CNNconfiguration
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
def CNNwo():
    mkdir("result")
    mkdir("model")
    ftest_loss = open("result/CNNwo_test_loss.txt","w")
    ftest_acc = open("result/CNNwo_test_accuracy.txt","w")
    ftrain_loss = open("result/CNNwo_train_loss.txt","w")
    ftrain_acc = open("result/CNNwo_train_accuracy.txt","w")
    ...
    config =  CNNconfiguration(vocab_size= 10000,embedding_size = 100,filter_num = 100,filter_length = 3,num_class = 1,sentence_length = 300,maxpool_kernel=4,batch_size = 10,pre_embed = False)          
    layer = TextCNN(config)
    ...
    print("CNNwo time %.3f:"%(end_t-start_t))
